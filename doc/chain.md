...
Бесконечная периодическая цепная дробь ```a+1/(b1+1/(b2+1/(b3+ ... + bN+{1/(c1+1/(c2+1/(c3 ... 1/cM}``` ... представляется в виде:  
```{a, [b1, b2, b3, ... bN], [c1, c2, ... cM]}```  
где:  
```a``` - целая часть дроби  
```b1, b2, b3, ... bN``` - непериодическая часть дроби  
```c1, c2, c3, ... cM``` - периодическая часть дроби  
