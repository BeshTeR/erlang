...
%% Типы ------------------------------------------------------------------------
-record(state, {events,    % список записей #event{}
        ...
        {done, Name}                                     ->
            case orddict:find(Name, S#state.events) of
                ...
                error ->
                    ...
                    %% и тут же пришло время ему сработать
                    loop(S)
            end;
        shutdown                                         ->
            exit(shutdown);
...
init() ->
    %% Здесь можно загрузить список событий изфайла на диске.
    %% Необходимо будет передать параметр в init, указывающий,
    %% где находится ресурс со списком событий. Затем загрузить его.
    %5 Другой вариант - передать готовый список событий серверу
    %% через эту функцию.
...
valid_datetime({Date, Time}) ->
    try
        ...
        error:function_clause -> % не соответствует {{Г,М,Д}, {Ч,Мин,Сек}}
        false
    end;
valid_datetime(_) ->
    false.
